...
##'     user = "user", password = "password")
##' (crs <- CRS("+proj=longlat"))
##' pgSRID.SpatRaster(conn, crs)
##' (crs2 <- CRS(paste("+proj=stere", "+lat_0=52.15616055555555 +lon_0=5.38763888888889",
##'     "+k=0.999908 +x_0=155000 +y_0=463000", "+ellps=bessel",
##'     "+towgs84=565.237,50.0087,465.658,-0.406857,0.350733,-1.87035,4.0812",
##'     "+units=m")))
##' pgSRID.SpatRaster(conn, crs2, create.srid = TRUE)
##' }

pgSRID.SpatRaster <- function(conn, crs, create.srid = FALSE, new.srid = NULL) {
    ## Check if PostGIS is enabled
    dbConnCheck(conn)
    if (!suppressMessages(pgPostGIS(conn))) {
        stop("PostGIS is not enabled on this database.")
    ...
    ## check object
    if (!inherits(crs, "character")) {
        stop("Object is not a character string.")
    }
    ## extract p4s
    p4s <- crs
    ## if crs is undefined (NA), return 0
