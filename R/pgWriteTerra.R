...
##'
##' Sends R terra raster to a PostGIS database table.
##' 
...
##' @param raster A raster from the 
##'     terra package.
...
##' @examples
##' \dontrun{
##' pgWriteTerra(conn, c("schema", "tablename"), raster_name)
##'
##' # basic test
##' r <- raster::raster(nrows=180, ncols=360, xmn=-180, xmx=180,
##'     ymn=-90, ymx=90, vals=1)
##' pgWriteTerra(conn, c("schema", "test"), raster = r,
##'     bit.depth = "2BUI", overwrite = TRUE)
##' }

pgWriteTerra <- function(conn, name, raster, bit.depth = NULL, 
    ...
    # crs
    r_crs <- dbQuoteString(conn, as.character(terra::crs(raster)))
    ...
    if (!is.null(blocks)) {
      ...
      tr <- tbs(r1[[1]], 10000, minblocks = 1, minrows = 100)
      cr <- tbs(terra::t(r1[[1]]), 10000, minblocks = 1, 
        minrows = 100)
    }
...
#' terra implementation of raster::blockSize
tbs = function (x, ...) 
{
    .local <- function (x, chunksize, n = dim(x)[3], minblocks = 4, 
        minrows = 1) 
    {
        n <- max(n, 1)
        ...
        #blockrows <- try(methods::slot(x@file, "blockrows"), 
        #    silent = TRUE)
        ...
        nr <- terra::nrow(x)
        size <- min(nr, max(1, floor(bs/(terra::ncol(x) * n * 8))))
        if (size > 1) {
